...
const getOrderDetails = async (params) =>
  await get(url.GET_ORDER_MANAGEMENT + "/specific/" + params);
...
const deleteProductList = async (payload) =>
  await del(url.DELETE_PRODUCT_LIST + "/" + payload);
...
const deleteSalesOrderList = (payload) =>
  del(url.DELETE_ORDER_MANAGEMENT + "/" + payload);
...
const getContactDetails = async (payload) =>
  await get(url.GET_SPECIFIC_CONTACT + "/" + payload);

// Auth Login
const Login = (payload) => postwithnotoken(url.LOGIN, payload);

// Auth Verify Email
const VerifyEmailCall = (payload) => postwithnotoken(url.VERIFY_EMAIL, payload);

// Auth Reset Password
const ResetPasswordCall = (payload) =>
  postwithnotoken(url.RESET_PASSWORD, payload);
...
const postAzureLogin = async (payload) =>
  await postwithnotoken(url.AZURE_LOGIN, payload);

const ChangePasswordCall = async (payload) =>
  await post(url.CHANGE_PASSWORD, payload);
...
const updateUserCall = (payload) =>
  patch(url.USERS + "/" + payload.code, payload.data);
...
const getInvoicesCall = async (params) => await get(url.GET_INVOICES + params);

const getSpecificInvoiceCall = async (params) =>
  await get(url.GET_INVOICES + "/specific/" + params);

const getAccountStatement = async (params) =>
  await get(url.ACCOUNT_STATEMENT + params);

// getCreditCardInfoCall,
// getBankAccountInfoCall,
// postPaymentJournalCall

const getCreditCardInfoCall = async (params) =>
  await get(url.GET_CREDIT_CARD_INFO);
const getBankAccountInfoCall = async (params) =>
  await get(url.GET_BANK_ACCOUNT_INFO);
const postPaymentJournalCall = async (params) =>
  await post(url.CREATE_PAYMENT_JOURNAL, params);

// addBankAccountCall,
// addCreditCardCall

const addBankAccountCall = async (params) =>
  await post(url.ADD_BANK_ACCOUNT, params);

const addCreditCardCall = async (params) =>
  await post(url.ADD_CREDIT_CARD, params);

const getAvailableCardsCall = async (params) =>
  await get(url.GET_AVAILABLE_CARDS);
